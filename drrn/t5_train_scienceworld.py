...
from t5_agent import T5Agent
from vec_env import VecEnv, AugmentedVecEnv
import random
...
def train(agent, envs, max_steps, update_freq, eval_freq, checkpoint_freq, log_freq, args, bufferedHistorySaverTrain, bufferedHistorySaverEval):
    ...
    prev_actions = [None for _ in range(envs.num_envs)]
    prev_observations = [None for _ in range(envs.num_envs)]
    states = agent.build_state(obs, infos, prev_actions, prev_observations)
    ...
    for step in range(1, max_steps+1):
        ...
        if step % log_freq == 0:            
            ...
            if isinstance(envs, AugmentedVecEnv):
                tb.logkv('numHits', np.array(envs.hits))
...
def parse_args():
    ...
    parser.add_argument('--model_name_or_path', type=str, help="path to trained T5 model")
    ...
    parser.add_argument("--use_augmented", action="store_true", help="use the aligned gpt3 augmented data")
    parser.add_argument("--train_dir", type=pathlib.Path, help="directory where aligned training data is stored")
    parser.add_argument("--use_task_idx", action="store_true", help="when getting votes from the count index, use the current task index. If false, uses votes across all tasks. ")
    parser.add_argument("--count_reward", type=float, default=5.0, help="reward for completing subsequence (will be multiplied by normalized vote count)")
...
def main():
    ...
    if args.use_augmented:
        envs = AugmentedVecEnv(args.num_envs, args)
    else:
